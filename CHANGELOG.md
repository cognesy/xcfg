...
- `resources/skills/use-xcfg/SKILL.md`: how to structure configuration with
  xcfg, led by the N x M layout mistake — a profile must be a *value* of a
  section, not a directory containing sections. Checked against the real
  surface by `tests/test_skill.py`, which caught three undocumented spec
  fields on first run.
